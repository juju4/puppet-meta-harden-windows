...
#    policy_value   => '*S-1-5-32-544,*S-1-5-11,*S-1-5-9',
  ...
  # applocker, module seems to manage only Exe and does not manage exceptions...
  ...
  windows_applocker { 'Allow Appveyor python':
    rule_type => 'path',
    rule_id => 600,
    app_path => '%OSDRIVE%\CYGWIN64\BIN\PYTHON3.6M.EXE'
    app_name => 'python3.6m.exe'
  }
  windows_applocker { 'Allow chocolatey (ProgramData)':
    rule_type => 'wildcard',
    rule_id => 300,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\ProgramData\chocolatey\bin\*',
  }
  windows_applocker { 'Allow chocolatey choco.exe (ProgramData)':
    rule_type => 'path',
    rule_id => 400,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\ProgramData\chocolatey\choco.exe',
    app_name => 'choco.exe'
  }
  windows_applocker { 'Allow chocolatey tools (checksum, 7z...)':
    rule_type => 'wildcard',
    rule_id => 500,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\ProgramData\chocolatey\tools\*'
  }
  windows_applocker { 'Allow chocolatey temp directory':
    rule_type => 'wildcard',
    rule_id => 600,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\Users\appveyor\AppData\Local\Temp\chocolatey\*'
  }
  windows_applocker { 'Allow Adobe reader install through chocolatey':
    rule_type => 'wildcard',
    rule_id => 600,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\ProgramData\Adobe\Setup\*'
  }
  windows_applocker { 'Allow chocolatey package installation from ProgramData':
    rule_type => 'wildcard',
    rule_id => 600,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\ProgramData\chocolatey\lib\*'
  }
#  windows_applocker { 'Allow DISMHOST.EXE':
#    rule_type => 'certificate', # FIXME! not supported...
#  }
  windows_applocker { 'Allow sysinternals tools through chocolatey':
    rule_type => 'wildcard',
    rule_id => 600,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\ProgramData\chocolatey\lib\sysinternals\tools\*'
  }
  windows_applocker { 'Allow %OSDRIVE%\temp\*':
    rule_type => 'wildcard',
    rule_id => 600,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\temp\*'
  }
  windows_applocker { 'Allow a Trusted Execution directory per user':
    rule_type => 'wildcard',
    rule_id => 600,
    app_path => '%OSDRIVE%\Users\*\TrustedPathExec\*'
  }
# certificate allow: cisco webex, gotomeeting...
  windows_applocker { 'Allow osqueryd':
    rule_type => 'wildcard',
    rule_id => 600,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\ProgramData\osquery\osqueryd\*'
  }
  windows_applocker { 'Allow Inspec':
    rule_type => 'wildcard',
    rule_id => 600,
    identity => 'Administrators',
    app_path => '%OSDRIVE%\opscode\inspec\*'
  }
